...
return db.query("select * from video_tbl where video_id=?",[id],callback);
 ...
 return db.query("Insert into video_tbl values(?,?,?,?,?)",[Video.video_id,Video.video_title,Video.video,Video.fk_u_email_id,Video.fk_sub_id],callback);
 },
deleteVideo:function(id,callback){
 ...
 updateVideo:function(id,Video,callback){
  return db.query("update video_tbl set video_title=?,video=? where video_id=?",[Video.video_title,Video.video,id],callback);
 }
